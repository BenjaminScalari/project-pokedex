...
// Definiamo le evoluzioni di Poliwag con Poliwrath e Politoed
const poliwagEvolutions = [
  { name: "poliwrath", id: 62 }, // ID di Poliwrath
  { name: "politoed", id: 186 }, // ID di Politoed
];
...
const PoliwagEvolutions = () => {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setCurrentEvolutionIndex(
        (prevIndex) => (prevIndex + 1) % poliwagEvolutions.length
      );
  ...
  const currentEvolution = poliwagEvolutions[currentEvolutionIndex];
...
export default PoliwagEvolutions;
